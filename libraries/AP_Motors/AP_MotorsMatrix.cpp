...
void AP_MotorsMatrix::output_armed_stabilizing()
{
    ...
    for (i = 0; i < AP_MOTORS_MAX_NUM_MOTORS; i++) {
        if (motor_enabled[i]) {
            ...
            avSUM=avSUM+_thrust_rpyt_out[i];
    ...
    log_params[9]=avSUM/6;
     ...
     AP::logger().Write("FWKS", "TimeUS,BRP1,thRo,thPi,thYa,thTh,thCo,avS1,thBs,avS2,avS3", "Qffffffffff",
                                        AP_HAL::micros64(),
                                        (double)log_params[0],
                                        (double)log_params[1],
                                        (double)log_params[2],
        ...
        if (counter > 8000) {
            ...
            switch (GCS_message_mixmode) {
                case 1: { 
                    ...
                    gcs().send_text(MAV_SEVERITY_EMERGENCY, "EMERGENCY MODE - LAND NOW!");
                }default: {
                    break;
                }
